...
ComplexTensorSpace::~ComplexTensorSpace()
{
...
bool ComplexTensorSpace::get_disk_cache()
{
    return disk_cache_;
}

char* ComplexTensorSpace::get_cache_name()
{
    return cache_name_;
}

int ComplexTensorSpace::get_num_site()
{
    return num_site_;
}

int ComplexTensorSpace::get_num_site_pp()
{
    return num_site_pp_;
}

int ComplexTensorSpace::get_num_site_mm()
{
    return num_site_mm_;
}

ComplexTensorLattice* ComplexTensorSpace::get_tensor_lattice(int site)
{
    return tensor_lattice_[site];
}

void ComplexTensorSpace::DefineTensorSpaceCache(bool disk_cache, char *cache_name)
{
    disk_cache_ = disk_cache;
    strcpy(cache_name_, cache_name);
}

void ComplexTensorSpace::DefineTensorSpaceParameter(int max_block, int max_dim, 
...
void ComplexTensorSpace::DefineTensorSpace(int initial_block, int initial_dim)
{
...
void ComplexTensorSpace::PrintTensorSpace()
{
...
void ComplexTensorSpace::WriteTensorSpace(const char* tensor_space_name)
{
...
void ComplexTensorSpace::WriteTensorSpace(ofstream &tensor_space_file)
{
...
void ComplexTensorSpace::ReadTensorSpace(const char* tensor_space_name)
{
...
void ComplexTensorSpace::ReadTensorSpace(ifstream &tensor_space_file)
{
    for(int i=0;i<num_site_pp_;++i)
    {
        for(int j=0;j<num_table_[i];++j)
    ...
    for(int i=0;i<num_site_;++i)
    {
        tensor_lattice_[i] = new ComplexTensorLattice();
...
void ComplexTensorSpace::ResetTensorSpace(int site)
{
    tensor_lattice_[site]->ResetTensorLattice();
}

void ComplexTensorSpace::CanonicalTensorSpace(int leigh, int site)
{
    if(leigh==0) 
...
void ComplexTensorSpace::MergeTensorSpace(int leigh, int site)
{
    if(leigh==0 && site>0)
...
void ComplexTensorSpace::ExpanTensorSpace(int leigh, int site)
{
...
void ComplexTensorSpace::ComputeLatticeBlock(int site, int &num_left_block, int &num_right_block, 
...
void ComplexTensorSpace::ComputeTensorIndex(int site, int &num_block, int* &left_index, int* &right_index, 
                         int* &physics_index)
{
...
void ComplexTensorSpace::ComputeExpanTensorLattice(int leigh, int site, int operator_num_table, 
        int **operator_quantum_table, int** &mapping_table)
{
    ...
    expan_tensor_lattice_ = new ComplexTensorLattice(physics_dim);
...
void ComplexTensorSpace::DefineQuantumTable()
{
    cout << "Base class do not define QuantumTable" << endl;
    exit(-1);
}

void ComplexTensorSpace::ReorderQuantumTable(int num_quantum, int num_table, int** quantum_table, 
                                          double *mean_quantum)
{
    double* deviate;
    int* index;
